...
CONFIG_DIR = "." 
# Se estiver em subpasta, mude para:
...
TABLES_FALLBACK = { "México": {"rates": {"ISR_Simplificado": 0.15, "IMSS_Simplificado": 0.05}}, "Chile": {"rates": {"AFP": 0.1115, "Saúde": 0.07}}, "Argentina": {"rates": {"Jubilación": 0.11, "Obra Social": 0.03, "PAMI": 0.03}}, "Colômbia": {"rates": {"Saúde": 0.04, "Pensão": 0.04}}, "Canadá": {"rates": {"Income Tax_Simplificado": 0.15}} }
EMPLOYER_COST_FALLBACK = {
...
COUNTRY_TABLES_FALLBACK = {"TABLES": TABLES_DEFAULT, "EMPLOYER_COST": EMPLOYER_COST_FALLBACK, "REMUN_MONTHS": REMUN_MONTHS_DEFAULT}
...
def get_sti_level_map(area: str, T: Dict[str, str]) -> Tuple[List[str], Dict[str, str]]:
    keys = STI_LEVEL_OPTIONS.get(area, []); display_list = [T.get(STI_I18N_KEYS.get(key, key), key) for key in keys] # Fallback para a própria chave
    return display_list, dict(zip(display_list, keys))
...
with st.sidebar:
    ...
    _COUNTRY_CODE_FOR_FMT = country # Atualiza global
...
# =========================== REGRAS DE CONTRIBUIÇÕES ===================
    ...
    # Define os dados da tabela ANTES de usá-los
