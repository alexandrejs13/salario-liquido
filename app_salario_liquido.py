# -------------------------------------------------------------
# 📄 Simulador de Salário Líquido e Custo do Empregador (v2025.50.45 - FIX FINAL ESTILO CARD HOMOGÊNEO)
# Correção: Cards anuais reescritos para usar a classe .metric-card diretamente, garantindo alinhamento e consistência visual.
# -------------------------------------------------------------
...
"annual_salary": "Salario Anual", # Emojis removidos
"annual_bonus": "Bono Anual", # Emojis removidos
"annual_total": "Remuneração Anual Total", # Emojis removidos
...
.annual-card-base {
    ...
    /* FIX 5: Adiciona espaçamento vertical entre os cards */
    margin-bottom: 10px; 
}
.annual-card-base h3 {
...
if active_menu == T.get("menu_calc"):
    ...
    # --- NOVO BLOCO DE CARDS ANUAIS (Unificado e Alinhado) ---
    with col_cards:
        
        # Função interna para renderizar cards anuais (usando o estilo metric-card)
        def render_annual_card(label_text, value_text, emoji, color_hex, bg_color_hex):
             st.markdown(f"""
            <div class='annual-card-base' style='
                border-left-color: {color_hex}; 
                background: {bg_color_hex};
                text-align: left;'>
                <h4>{emoji} {label_text}</h4>
                <h3 style='margin-top: 5px !important;'>{value_text}</h3>
            </div>
            """, unsafe_allow_html=True)
        ...
        render_annual_card(
            f"{T.get('annual_salary','Salário')} (1)",
            fmt_money(salario_anual, symbol),
            "📅",
            "#28a745",
            "#e6ffe6"
        )
        ...
        # Sequência 2: Bônus (2)
        render_annual_card(
            f"{T.get('annual_bonus','Bônus')} (2)",
            fmt_money(bonus_anual, symbol),
            "🎯",
            cor,
            bg_cor
        )
        ...
        render_annual_card(
            T.get('annual_total','Remuneração Total'),
            fmt_money(total_anual, symbol),
            "💼",
            "#0a3d62",
            "#e6f0f8"
        )

    # --- FIM NOVO BLOCO DE CARDS ANUAIS ---
