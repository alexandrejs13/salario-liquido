# -------------------------------------------------------------
# 📄 Simulador de Salário Líquido e Custo do Empregador (v2025.50.23 - REFINAMENTO DE LAYOUT)
# Correção: Estilização de rótulos, remoção de duplicidade (STI) e ajuste de layout Brasil.
# -------------------------------------------------------------
...
st.markdown("""
<style>
/* Ajuste no estilo dos títulos para ficarem menores e mais discretos */
.stMarkdown h5 {
    font-size: 14px; 
    font-weight: 500; 
    line-height: 1.2; 
...
if active_menu == T.get("menu_calc"):
    ...
    def get_sti_area_label(T):
        area_label = T.get('area', 'Área').replace(' (STI)', '').strip()
        return f"{area_label}<br>(STI)"

    def get_sti_level_label(T):
        level_label = T.get('level', 'Career Level').replace(' (STI)', '').strip()
        return f"{level_label}<br>(STI)"


    if country == "Brasil":
        # Layout Brasil: Agora segue o padrão de 4 campos na primeira linha e 2 STI na segunda.
        ...
        r1, r2, _ = st.columns([1, 1, 2]) # Largura 1:1 para picklists STI e 2 para espaço vazio
        ...
        dependentes_fixed = dependentes # Mantém a variável de dependentes
        ...
        r1, r2, _ = st.columns([1, 1.5, 2.5]) # Larguras desiguais para STI para dar mais espaço ao nível (títulos mais longos)
        area_display = r1.selectbox("Área STI", area_options_display, index=0, key="sti_area", help=T.get("sti_area_tooltip"), label_visibility="collapsed")
        area = area_display_map.get(area_display, "Non Sales")
        level_options_display, level_display_map = get_sti_level_map(area, T)
        level_default_index = len(level_options_display) - 1 if level_options_display else 0
