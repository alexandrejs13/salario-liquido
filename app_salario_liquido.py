# -------------------------------------------------------------
# 📄 Simulador de Salário Líquido e Custo do Empregador (v2025.50.40 - FIX FINAL REFINAMENTO)
# Correção: Layout Anual (Cards/Gráfico), Notações de Rodapé, Reposicionamento de FGTS e Divisores.
# -------------------------------------------------------------
...
/* 3. PADRONIZAÇÃO DE CARDS: Garante que os cards anuais sejam menores e sigam o estilo metric-card */
.annual-card-base {
    min-height: 70px !important; /* Altura mínima reduzida */
    padding: 6px 10px !important; /* Padding reduzido */
    display: flex;
...
.annual-card-base h3 {
    font-size: 16px !important; /* Fonte menor para valor */
    font-weight: 700;
    margin: 2px 0 0 !important;
}
.annual-card-base h4 {
    font-size: 14px !important; /* Fonte menor para título */
    font-weight: 600;
    margin: 0 !important;
}

/* 4. Estilo de Tabela (Igual ao .table-wrap) */
.stTable {
...
if active_menu == T.get("menu_calc"):
    ...
    sti_line = f"STI ratio do bônus: <strong>{pct_txt}</strong> — <strong>{status_txt}</strong> ({faixa_txt})" # Removido o fim para a nota
    ...
    # 3) e 2) NOTAS E FGTS ABAIXO DO LAYOUT ANUAL
    # Notas do Salário Anual e Bônus
    st.markdown(f"""
    <p style="margin-top: -10px; font-size: 14px; color: #555;">
        (1) (Meses considerados: {months})
    </p>
    <p style="margin-top: -10px; font-size: 14px; color: #555;">
        (2) {T.get('annual_bonus','Bônus Anual')} details: {sti_line}
    </p>
    """, unsafe_allow_html=True)
    
    # 3) FGTS ABAIXO DOS CARDS MENSAIS (remanejado para aqui para melhor fluxo)
    if country == "Brasil": 
        st.markdown(f"""
        <div style="margin-top: 20px; padding: 5px 0;">
            <p style="font-size: 17px; font-weight: 600; color: #0a3d62;">
