# -------------------------------------------------------------
# 📄 Simulador de Salário Líquido e Custo do Empregador (v2025.50.19)
# Tema azul plano, multilíngue, responsivo e com STI corrigido
# (Correção Crítica: KeyError/NameError na definição de Fallbacks)
# -------------------------------------------------------------
...
def calc_employer_cost(country_code: str, salary: float, bonus: float, T: Dict[str, str], tables_ext=None):
    ...
    if not df.empty:
        ...
        df_display[T["cost_header_obs"]] = df_display.apply(lambda row: fmt_cap(row.get('teto'), COUNTRIES[country_code]['symbol']) if row.get('teto') is not None else row['obs'], axis=1)
...
# REQ 1 (Correção): Acessa com .get() para evitar KeyError se a chave 'country' não for válida
