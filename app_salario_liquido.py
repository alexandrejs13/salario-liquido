...
/* Chips exibidos no final da sidebar: ocultar */
.sidebar-chip{ display:none; }

/* ----- Cards compactos da Composição Anual (mesmo estilo dos cards de totais) ----- */
...
def br_net(
    salary: float, dependentes: int, br_inss_tbl: Dict[str, Any], br_irrf_tbl: Dict[str, Any]
):
    lines = []
    total_earn = salary
...
US_STATE_RATES, COUNTRY_TABLES, BR_INSS_TBL, BR_IRRF_TBL, OK_REMOTE = load_tables(
...
if menu == T["menu_calc"]:
    if country == "Brasil":
        ...
        state_rate = c3.number_input(
            f"{T['state_rate']}", min_value=0.0, max_value=0.20, value=default_rate, step=0.001, format="%.3f", key="state_rate_input"
        )
    ...
    cc1.markdown(
        f"<div class='metric-card'><h4>🟩 {T['tot_earnings']}</h4><h3>{fmt_money(calc['total_earn'], symbol)}</h3></div>",
        unsafe_allow_html=True,
    )
    cc2.markdown(
        f"<div class='metric-card'><h4>🟥 {T['tot_deductions']}</h4><h3>{fmt_money(calc['total_ded'], symbol)}</h3></div>",
        unsafe_allow_html=True,
    )
    cc3.markdown(
        f"<div class='metric-card'><h4>🟦 {T['net']}</h4><h3>{fmt_money(calc['net'], symbol)}</h3></div>",
        unsafe_allow_html=True,
    )
...
# ---- Cards horizontais, estilo igual aos cards superiores ----
...
c2.markdown(
    f"<div class='metric-card'><h4>🎯 {T['annual_bonus']}</h4>"
    f"<h3>{fmt_money(bonus_anual, symbol)}</h3>"
    f"<div class='sti-note' style='color:{cor}; margin-top:6px;'>"
    f"STI ratio do bônus: <strong>{pct_txt}</strong> — <strong>{status_txt}</strong> ({faixa_txt}) — "
    f"<em>{area} • {level}</em></div></div>",
    unsafe_allow_html=True,
)
...
chart_df = pd.DataFrame(
    {"Componente": [T["annual_salary"], T["annual_bonus"]], "Valor": [salario_anual, bonus_anual]}
)
...
chart = (
    ...
    .properties(width=500, height=350, title=T["pie_title"])
    .configure_legend(orient="bottom", padding=10)
    .configure(padding={"left": 0, "right": 0, "top": 10, "bottom": 80})
)
...
# =========================== REGRAS DE CONTRIBUIÇÕES ===================
    ...
    if country == "Brasil":
        st.markdown(
            """
**Empregado (Brasil)**  
- **INSS (progressivo)**: somatório por faixas até o salário, com **teto de contribuição**.  
- **IRRF**: base = **salário bruto − INSS − dedução por dependentes**; aplica-se a **alíquota** da faixa e a **dedução fixa**.  
- **FGTS**: depósito de 8% pelo empregador (não entra como desconto do empregado).
"""
        )
        ...
        st.markdown(
            """
**Empregador (Brasil)**  
- **INSS Patronal, RAT, Sistema S** sobre folha (percentuais variam por CNAE).  
- **FGTS (8%)** como depósito. Em geral incidem sobre **férias** e **13º**, compondo o custo anual (meses ≈ **13,33**).
"""
        )
    elif country == "Estados Unidos":
        st.markdown(
            """
**Empregado (EUA)**  
- **FICA**: 6,2% (Social Security) até o wage base.  
- **Medicare**: 1,45% (sem teto).  
- **State Tax**: conforme o estado escolhido (alíquota ajustável).
"""
        )
        st.markdown(f"### {T['rules_er']}")
        st.markdown(
...
- Meses de remuneração: **12** (não há 13º/adição de férias no padrão federal).
"""
        )
    elif country == "México":
        st.markdown(
