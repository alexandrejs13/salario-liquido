# -------------------------------------------------------------
# 📄 Simulador de Salário Líquido e Custo do Empregador (v2025.50.19 - FINAL FIX)
# Tema azul plano, multilíngue, responsivo e com STI corrigido
# (Correção Crítica: Ordem Título Sidebar e Carregamento STI)
# -------------------------------------------------------------
...
# CORREÇÃO CRÍTICA STI: Se o STI_CONFIG_DATA não for o formato FALLBACK, ele está no formato I18N. 
# Usamos o FALLBACK para as faixas se o arquivo falhou na carga (o que é mais seguro).
# O problema é que o STI_CONFIG enviado continha I18N, mas o FALLBACK não.
# Vamos assumir o formato de fallback para as faixas se a carga falhar.
if not STI_CONFIG_DATA or "STI_RANGES" not in STI_CONFIG_DATA:
    # Se o arquivo STI_CONFIG falhar ou tiver o formato antigo (I18N), usamos o FALLBACK
    STI_RANGES = STI_CONFIG_FALLBACK.get("STI_RANGES", {})
    STI_LEVEL_OPTIONS = STI_CONFIG_FALLBACK.get("STI_LEVEL_OPTIONS", {})
else:
    # Se o STI_CONFIG_DATA foi carregado corretamente no formato não-i18n
...
with st.sidebar:
    ...
    # Recarrega T com o idioma final
