# -------------------------------------------------------------
# 📄 Simulador de Salário Líquido e Custo do Empregador (v2025.50.51 - CORREÇÕES FINAIS DE UI E TRADUÇÃO)
# CORREÇÃO: Lógica de menu refatorada com chaves internas para garantir a tradução correta do título da página.
# CORREÇÃO: Adicionadas traduções ausentes na página "Custo do Empregador".
# FEATURE/CSS: A barra lateral agora tem largura fixa (350px) e não pode ser redimensionada pelo usuário.
# -------------------------------------------------------------
...
with st.sidebar:
    ...
    if idioma != st.session_state.idioma:
        ...
        st.session_state.active_menu_key = "calc" # Reset to default page on language change
    ...
    menu_labels = [T.get(f"menu_{key}") for key in MENU_KEYS]
    
    if 'active_menu_key' not in st.session_state:
        st.session_state.active_menu_key = "calc"
    
    try:
        current_index = MENU_KEYS.index(st.session_state.active_menu_key)
    except ValueError:
        current_index = 0
        st.session_state.active_menu_key = "calc"

    selected_label = st.radio("Menu Select", options=menu_labels, index=current_index, label_visibility="collapsed", key="menu_radio_widget")
