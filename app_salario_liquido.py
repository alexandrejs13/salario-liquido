# -------------------------------------------------------------
# 📄 Simulador de Salário Líquido e Custo do Empregador (v2025.50.20 - FIX RESPONSIVIDADE)
# Correção do alinhamento e títulos dos campos na seção de parâmetros
# -------------------------------------------------------------
...
if active_menu == T.get("menu_calc"):
    ...
    # CORREÇÃO: Usando st.columns(6) e st.columns(5) para forçar colunas de largura IGUAL
    # Isso melhora a responsividade e a quebra de linha dos títulos
    ...
    if country == "Brasil":
        ...
        # 5 campos: Salário, Estado, Taxa Estadual, Outras Ded, Bônus
        ...
        # STI fields em uma nova linha com 2 colunas
        r1, r2, _ = st.columns([1, 1.5, 2.5]) # Manter a proporção para 2 campos e um vazio
        ...
        # 3 campos: Salário, Outras Ded, Bônus
        c1, c2, c3, _ = st.columns([1, 1, 1, 1]) # 3 campos e 1 coluna vazia para alinhar
        salario = c1.number_input(f"{T.get('salary', 'Salary')} ({symbol})", min_value=0.0, value=10000.0, step=100.0, key="salary_input", help=T.get("salary_tooltip"))
