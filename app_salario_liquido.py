# -------------------------------------------------------------
# 📄 Simulador de Salário Líquido e Custo do Empregador (v2025.50.32 - REFINAMENTO LAYOUT ANUAL)
# Correção: Layout Anual alterado para Cards (Esquerda) + Gráfico de Pizza (Direita).
# -------------------------------------------------------------
...
div.block-container {
    max-width: 1100px; /* Reduzido de 1400px para 1100px para visual mais elegante em telas grandes */
    padding-left: 1rem;
    padding-right: 1rem;
}

/* 2. AJUSTE NO ESTILO DOS TÍTULOS (MANTIDO) */
.stMarkdown h5 {
    font-size: 14px; 
    font-weight: 500; 
    line-height: 1.2; 
...
/* 3. Estilizando cards anuais para ficarem menores na visualização */
.annual-card-base {
    min-height: 70px !important; /* Reduz a altura mínima dos cards */
    padding: 6px 10px !important; /* Reduz o padding interno */
}
.annual-card-base h3 {
    font-size: 16px !important; /* Reduz o tamanho da fonte do valor */
}
.annual-card-base h4 {
    font-size: 13px !important; /* Reduz o tamanho da fonte do título */
}
...
.annual-card-value h3 { margin: 0; color: #0a3d62; }
...
if active_menu == T.get("menu_calc"):
    ...
    with col_cards:
        # Cards de Remuneração Anual (Reduzidos)
        st.markdown(f"<div class='annual-card-base annual-card-label' style='border-left-color: #28a745; background: #e6ffe6;'><h4>{T.get('annual_salary','Annual Sal.')}</h4><span class='sti-note'>({T.get('months_factor','Months')}: {months})</span></div>", unsafe_allow_html=True)
        st.markdown(f"<div class='annual-card-base annual-card-value' style='border-left-color: #28a745; background: #e6ffe6;'><h3>{fmt_money(salario_anual, symbol)}</h3></div>", unsafe_allow_html=True)
        
        st.markdown(f"<div class='annual-card-base annual-card-label' style='border-left-color: {cor}; background: {bg_cor};'><h4>{T.get('annual_bonus','Annual Bonus')}</h4><span class='sti-note' style='color:{cor}'>{sti_line}</span></div>", unsafe_allow_html=True)
        st.markdown(f"<div class='annual-card-base annual-card-value' style='border-left-color: {cor}; background: {bg_cor};'><h3>{fmt_money(bonus_anual, symbol)}</h3></div>", unsafe_allow_html=True)

        st.markdown(f"<div class='annual-card-base annual-card-label' style='border-left-color: #0a3d62; background: #e6f0f8;'><h4>{T.get('annual_total','Annual Total')}</h4></div>", unsafe_allow_html=True)
        st.markdown(f"<div class='annual-card-base annual-card-value' style='border-left-color: #0a3d62; background: #e6f0f8;'><h3>{fmt_money(total_anual, symbol)}</h3></div>", unsafe_allow_html=True)
    ...
    with col_chart:
        ...
        # Tenta pegar o nome curto do salário e bônus para o rótulo
        ...
        base = alt.Chart(chart_df).transform_joinaggregate(
            ...
            Label=alt.expr.if_(alt.datum.Valor > alt.datum.Total * 0.05, 
                                alt.datum.Componente + " (" + alt.format(".1%")(alt.datum.Percent) + ")", 
                                "") # Rótulo se > 5%
        )
    ...
    # Restante das informações anuais (FGTS)
