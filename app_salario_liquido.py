# -------------------------------------------------------------
# 📄 Simulador de Salário Líquido e Custo do Empregador (v2025.50.54 - REDESIGN COMPLETO E CORREÇÕES FINAIS)
# DESIGN: Aplicada nova paleta de cores (#1470AF, #EBF1F7) em todo o app.
...
.monthly-table thead th { background-color: var(--card-bg); color: var(--text-color); padding: 12px 15px; text-align: left; font-weight: 600; }
...
if active_menu_key == "calc":
    ...
    if country == "Brasil": st.markdown(f"<div style='margin-top: 10px; padding: 5px 0;'><p style='font-size: 17px; font-weight: 600; color: #1470AF;'>💼 {T.get('fgts_deposit')}: {fmt_money(calc['fgts'], symbol)}</p></div>", unsafe_allow_html=True)
    st.write("---")
    ...
    cor = "#1470AF" if dentro else "#d32f2f"; status_txt = T.get("sti_in_range") if dentro else T.get("sti_out_range")
    ...
    st.markdown(f"<div style='margin-top: 10px; padding: 5px 0;'><p style='font-size: 17px; font-weight: 600; color: #1470AF; margin: 0;'>📅 {T.get('months_factor')}: {months}</p><p style='font-size: 17px; font-weight: 600; color: #1470AF; margin: 5px 0 0 0;'>🎯 STI Ratio: {sti_note_text}</p></div>", unsafe_allow_html=True)
    st.write("---")
    ...
    pie = base.mark_arc(outerRadius=120, innerRadius=80, cornerRadius=2).encode(theta=alt.Theta("Valor:Q", stack=True), color=alt.Color("Componente:N", legend=None, scale=alt.Scale(range=['#1470AF', '#D3D3D3'])), order=alt.Order("Percent:Q", sort="descending"), tooltip=[alt.Tooltip("Componente:N"), alt.Tooltip("Valor:Q", format=",.2f")])
    ...
    final_chart = alt.layer(pie, text).properties(title=T.get("pie_chart_title_dist")).configure_view(strokeWidth=0).configure_title(fontSize=17, anchor='middle', color='#1470AF')
...
# ========================= COMPARADOR DE REMUNERAÇÃO ==========================
    ...
    with res_prop:
        ...
        st.markdown("</div>", unsafe_allow_html=True)
        if country == "Brasil": st.markdown(f"<p style='font-size: 15px; text-align: center; font-weight: 600; color: #1470AF;'>💼 {T.get('fgts_deposit')}: {fmt_money(calc_prop['fgts'], symbol)}</p>", unsafe_allow_html=True)
    ...
    with res_cand:
        ...
        st.markdown("</div>", unsafe_allow_html=True)
        if country == "Brasil": st.markdown(f"<p style='font-size: 15px; text-align: center; font-weight: 600; color: #1470AF;'>💼 {T.get('fgts_deposit')}: {fmt_money(calc_cand['fgts'], symbol)}</p>", unsafe_allow_html=True)
