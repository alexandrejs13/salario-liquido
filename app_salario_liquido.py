...
# (Correção Crítica: Ordem de Carregamento e Caminho de Arquivos)
# -------------------------------------------------------------
...
I18N_FILE = os.path.join(APP_DIR, "i18n.json")
COUNTRIES_FILE = os.path.join(APP_DIR, "countries.json")
STI_CONFIG_FILE = os.path.join(APP_DIR, "sti_config.json")
US_STATES_FILE = os.path.join(APP_DIR, "us_state_tax_rates.json")
COUNTRY_TABLES_FILE = os.path.join(APP_DIR, "country_tables.json")
BR_INSS_FILE = os.path.join(APP_DIR, "br_inss.json")
BR_IRRF_FILE = os.path.join(APP_DIR, "br_irrf.json")

def load_json(filepath, default_value={}):
    if not os.path.exists(filepath):
        # Este erro é crucial se os arquivos principais estiverem faltando
        st.error(f"Arquivo de configuração essencial não encontrado: {filepath}")
...
# --- Fallbacks Mínimos (apenas se o carregamento falhar) ---
...
# ============================== SIDEBAR ===============================
# A maior parte da lógica da sidebar foi movida para o topo
with st.sidebar:
    # REQ 1 (v. anterior): Título H3 para Idioma
    st.markdown(f"<h3 style='margin-bottom: 0.5rem;'>{I18N.get('Português', {}).get('language_title', 'Idioma')}</h3>", unsafe_allow_html=True) # Fallback
    idioma = st.selectbox(label="Language Select", options=list(I18N.keys()), index=0, key="lang_select", label_visibility="collapsed")
    T = I18N.get(idioma, I18N_FALLBACK["Português"]) # Define T baseado na seleção

    # REQ 5 (v. anterior): Título Traduzido - Renderiza DEPOIS de T
    ...
    # Gerencia o estado do país para evitar resetar com a mudança de idioma
    if 'country_select' not in st.session_state:
        st.session_state.country_select = default_country
    
    # Se o país salvo não estiver na lista (improvável), reseta
    if st.session_state.country_select not in country_options:
        st.session_state.country_select = default_country

    country_index = country_options.index(st.session_state.country_select)
    ...
    # Atualiza a variável global para fmt_cap
    ...
    # Gerencia o estado do menu para evitar resetar
    ...
    except ValueError: # Ocorre se o texto do menu mudou (idioma) e o estado salvo é antigo
        active_menu_index = 0
        st.session_state.active_menu = menu_options[0]
...
# Define variáveis globais com base no país selecionado
...
st.markdown(f"<div class='country-header'><div class='country_title'>{title}</div><div class='country-flag'>{flag}</div></div>", unsafe_allow_html=True)
st.write("---")
